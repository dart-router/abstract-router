...
class RouteTree {
  ...
  // 根据路径在树的分支上生成一个
  addRoute(final String path, final RouteHandler handler, [String method = DEFAULT_METHOD]) {
    ...
    for (int i = 0, len = nodeKeys.length; i < len; i ++) {
      ...
      if(node.type == NodeType.wildcard && i < len -1) {
        throw UnsupportRouteIdException(path, 'The * segment must be end of segments!');
      }
  ...
  void addMiddlewares(String path, List<RouteMiddleware> middlewares, [String method = DEFAULT_METHOD]) {
    ...
    if(path == '/') {
      ...
      root.middlewares[method].addAll(middlewares.map((RouteMiddleware middleware) => RouteMiddlewareWrap(_middlewareSeq ++, middleware)));
      return ;
    }
    ...
    for (int i = 0, len = nodeKeys.length; i < len; i ++) {
      ...
      if(i == len - 1) {
        ...
        // 此处所有的节点都不会有处理器
        node.middlewares[method].addAll(middlewares.map((RouteMiddleware middleware) => RouteMiddlewareWrap(_middlewareSeq ++, middleware)));
      }
  ...
  RouterContext getContext(String path, [String method = DEFAULT_METHOD]) {
    ...
    if(matchedResults.isEmpty) { // 匹配完成之后，并未匹配到记录
      return null;
    }

    // 已经匹配上了，根据匹配结果集构造路由上下文
    RouterContext context = _generateRouterContext(pathParts, matchedResults, method);
    context.path = path;
    context.queries = uri.queryParametersAll.map((String key, List<String> value) => MapEntry(key, value.length == 1 ? value[0] : value));

    return context;
  }

  RouterContext _generateRouterContext(final Stack<String> pathParts, final Stack<MatchResult> matchedResults, final String method) {
    ...
    middlewareWraps.sort((RouteMiddlewareWrap wm1, RouteMiddlewareWrap wm2) => wm1.seq - wm2.seq);
    context.middlewares.addAll(middlewareWraps.map((RouteMiddlewareWrap middlewareWrap) => middlewareWrap.handleFunc));
    context.params = params;
    
    return context;
  }

  _BacktrackingState _backtrackingCheck(final Stack<String> pathParts, final Stack<MatchResult> matchedResults) {
    print('back');
    MatchResult lastMatchResult;
    while(matchedResults.isNotEmpty) {
      lastMatchResult = matchedResults.top;
      ...
      } else if(lastMatchResult.wildcardCanBacktracking) { // 有通配节点可回溯
        return _BacktrackingState.yes;
      } else { // 当前帧不可回溯
